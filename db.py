...
for i in range(25344):
    temp = respon['response']['body']['items']['item'][i]  # i+623
    if "addr1" in temp:
        add = temp['addr1']
        if "addr2" in temp:
    ...
    commonvalue = (
    str(accomcount), str(chkbabycarriage), str(chkcreditcard), str(chkpet), str(infocenter), str(parking),
    str(restdate), int(contentid))
